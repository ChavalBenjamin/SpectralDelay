...
> This file is duplicated by `../duplicate.py` along with the rest of the project, and the project-name string-rewrite pass runs over it — so `SpectralDelay.cpp` etc. resolve to the new project's filenames after duplication.

## UI: responsive IGraphics layout

The UI in `SpectralDelay.cpp` is a **resizable / responsive IGraphics** example. The pattern is the important thing to preserve when extending it.
...
- `SpectralDelay.cpp` / `.h` — plugin class, parameters (`EParams`), control tags (`ECtrlTags`), `ProcessBlock`, `mLayoutFunc`.
