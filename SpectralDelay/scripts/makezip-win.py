...
def main():
  ...
  vst3_bundle = build_dir + "\\SpectralDelay.vst3"
  ...
  if not zip:
    installer = "\\build-win\\installer\\SpectralDelay Installer.exe"

    if demo:
      installer = "\\build-win\\installer\\SpectralDelay Demo Installer.exe"

    files = [
      projectpath + installer,
      projectpath + "\\installer\\changelog.txt",
      projectpath + "\\installer\\known-issues.txt",
      projectpath + "\\build-win\\manual\\SpectralDelay manual.pdf"
    ]
    ...
    vst3_bundle = projectpath + "\\build-win\\SpectralDelay.vst3"
    if os.path.exists(vst3_bundle):
      add_folder_to_zip(zf, vst3_bundle, "SpectralDelay.vst3")

    # Add standalone executables
    files = [
      projectpath + "\\build-win\\SpectralDelay_x64.exe",
      projectpath + "\\build-win\\SpectralDelay_ARM64EC.exe",
    ]
    ...
    clap_files = [
      # Postbuild locations
      (projectpath + "\\build-win\\SpectralDelay_x64.clap", "SpectralDelay_x64.clap"),
      (projectpath + "\\build-win\\SpectralDelay_ARM64EC.clap", "SpectralDelay_ARM64EC.clap"),
      # Build output locations (fallback)
      (projectpath + "\\build-win\\clap\\x64\\Release\\SpectralDelay.clap", "SpectralDelay_x64.clap"),
      (projectpath + "\\build-win\\clap\\ARM64EC\\Release\\SpectralDelay.clap", "SpectralDelay_ARM64EC.clap"),
    ]
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\SpectralDelay-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\SpectralDelay-vst3_ARM64EC.pdb",
    projectpath + "\\build-win\\pdbs\\SpectralDelay-app_x64.pdb",
    projectpath + "\\build-win\\pdbs\\SpectralDelay-app_ARM64EC.pdb",
    projectpath + "\\build-win\\pdbs\\SpectralDelay-clap_x64.pdb",
    projectpath + "\\build-win\\pdbs\\SpectralDelay-clap_ARM64EC.pdb"
  ]
